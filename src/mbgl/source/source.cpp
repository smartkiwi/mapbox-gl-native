...
bool Source::update(const StyleUpdateParameters& parameters) {
    ...
    int32_t overscaledZoom = coveringZoomLevel(parameters.transformState.getZoom(), type, tileSize);

    std::set<UnwrappedTileID> idealTiles;
    if (overscaledZoom >= info->minZoom) {
        ...
        idealTiles = tileCover(parameters.transformState, idealZoom);
   }
    ...
    for (const auto& unwrappedTileID : idealTiles) {
        const OverscaledTileID dataTileID(overscaledZoom, unwrappedTileID.canonical);
